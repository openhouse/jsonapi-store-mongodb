...
MongoStore._checkMinServerVersion = function() {
  ...
  if (semver.lt(serverVersion, MIN_SERVER_VERSION)) {
    throw new Error(
      'This version of jsonapi-store-mongodb requires jsonapi-server>=' +
        MIN_SERVER_VERSION +
        '.'
    );
  }
...
MongoStore._isRelationshipAttribute = function(attribute) {
  return (
    attribute._settings &&
    (attribute._settings.__one || attribute._settings.__many)
  );
};
...
MongoStore._getRelationshipAttributeNames = function(attributes) {
  ...
  var relationshipAttributeNames = attributeNames.reduce(function(
    partialAttributeNames,
    name
  ) {
...
MongoStore.prototype._createIndexesForRelationships = function(
  ...
  if (
    !Array.isArray(relationshipAttributeNames) ||
    !relationshipAttributeNames.length
  ) {
    return;
  }
...
MongoStore.prototype._applyPagination = function(request, cursor) {
  ...
  return cursor
    .skip(request.params.page.offset)
...
MongoStore.prototype.initialise = function(resourceConfig) {
  ...
  mongodb.MongoClient.connect(self._config.url, {
    ...
    .then(function() {
      ...
      self._createIndexesForRelationships(
        collection,
        self.relationshipAttributeNames
      );
      self.ready = true;
    });
};
...
MongoStore.prototype.find = function(request, callback) {
  ...
  collection.findOne({ _id: documentId }, { _id: 0 }, function(err, result) {
    if (err || !result) {
      return callback(
        MongoStore._notFoundError(request.params.type, request.params.id)
      );
    }
